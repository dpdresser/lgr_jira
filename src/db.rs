...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        ...
        serde_json::from_reader(db_file).map_err(|e| e.into())
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let db_file = File::create(&self.file_path)?;
        serde_json::to_writer(db_file, db_state).map_err(|e| e.into())
    }
}
...
mod tests {
    ...
    mod database {
        ...
        fn read_db_should_fail_with_invalid_path() {
            let db = JSONFileDatabase { file_path: "INVALID_PATH".to_string() };
